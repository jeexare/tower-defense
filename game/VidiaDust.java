/**
 * Constructs a VidiaDust GameObject.
 * 
 * 
...
public class VidiaDust extends GameObject {
	double deltaX;
	double deltaY;
	double angle;
	double speed = 8;
	...
	public VidiaDust(State state, Control control, Point posTower) {
	...
	public void update(double elapsedTime) {
		...
		if (currentX >= posEnemy.x - 30 && currentX <= posEnemy.x + 30
			...
			// If the enemy is a CaptainHook
			if (myEnemy instanceof CaptainHook
			&& myEnemy.isHit >= 10) {
			myEnemy.isExpired = true; // Expire the enemy
			control.SoundPlayer("src/resources/drown.wav");
			state.setScore(state.getScore() + 1000); // Add coins
	...
	public void draw(Graphics g) {
		...
		g.fillOval((int) currentX, (int) currentY, 20, 20);
