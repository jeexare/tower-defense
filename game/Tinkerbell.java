...
public class Tinkerbell extends GameObject
	...
	public Tinkerbell(State state, Control control) {
		...
		isVisible = true;
		isExpired = false;
		control.SoundPlayer("src/resources/swoosh.wav");
	}
		...
		y = control.getMouseY();
			...
			if (cooldownTime <= 0) {
				...
				if (state.getNearestEnemy(new Point(x, y)) == null) cooldownTime = 2.0; 
					...
					state.addGameObject(new TinkerDust(state, control, new Point(x, y)));
					cooldownTime = 2.0;
...
//						
	...
	public void draw(Graphics g) {
		g.drawImage(control.getImage("tinkerbell.png"), x, y, null);
		}
