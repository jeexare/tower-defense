/**
 * Constructs a TinkerDust GameObject.
 * 
 * 
...
public class TinkerDust extends GameObject {
	double deltaX;
	double deltaY;
	double angle;
	double speed = 8;
	...
	public TinkerDust(State state, Control control, Point posTower) {
		...
		control.SoundPlayer("src/resources/chimes.wav");
	...
	public void update(double elapsedTime) {
		...
		if (currentX >= posEnemy.x - 30 && currentX <= posEnemy.x + 30
			...
			// If the enemy is a WendyDarling and has been hit twice
			if (myEnemy instanceof WendyDarling
				&& myEnemy.isHit >= 2) {
				myEnemy.isExpired = true; // Expire the enemy
				control.SoundPlayer("src/resources/drown.wav");
				state.setScore(state.getScore() + 150); // Add coins
				state.setCoins(state.getCoins() + 150); // Add score
			}
			
			
			// If the enemy is a Crocodile and has been hit five times
	...
	public void draw(Graphics g) {
		g.setColor(new Color(252, 243, 213));
		g.fillOval((int) currentX, (int) currentY, 2, 2);
		g.fillOval((int) currentX + 3, (int) currentY + 3, 1, 1);
