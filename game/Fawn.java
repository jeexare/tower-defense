...
public class Fawn extends GameObject
	...
	private double cooldownTime = 2.0; // Time not allowed to fire after being placed of firing once
	private double waitTime = 2.0;
	 /** 
	  * Constructor, initializes the percentage and 
	  * state of 'this' Fawn object
	  * 
	  * @param state the state class
	  * @param control the control class
	  */
	    
	public Fawn(State state, Control control) {
	...
	public void update(double elapsedTime) {
		...
		y = control.getMouseY();
			...
			if (cooldownTime <= 0) {
				...
				if (state.getNearestEnemy(new Point(x, y)) == null) cooldownTime = 2.0; 
					...
					state.addGameObject(new FawnDust(state, control, new Point(x, y)));
					cooldownTime = 2.0;
...
//						
	...
	/**
	 * Draws the Fawn image.
	 */
	public void draw(Graphics g) {
		g.drawImage(control.getImage("fawn.png"), x, y, null);}
